...
test_delete() {
    ...
    assertFalse "$(map::has test_map key1)"
    assertTrue "$(map::has test_map key2)"
}
...
test_exists() {
    map::make test_map ""
    assertFalse "$(map::exists test_map)"

    map::set test_map key1 value1
    assertTrue "$(map::exists test_map)"
}
...
test_has() {
    ...
    assertTrue "$(map::has test_map key1)"
    assertTrue "$(map::has test_map key2)"
    assertFalse "$(map::has test_map key3)"
}
